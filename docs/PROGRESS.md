...
**Found, not fixed**
1. **The 781–1135px overflow band is still open** — Session 2 measured it, recommended raising the
   drawer breakpoint from 780px to ~1140px, and correctly called it a design decision rather than
   a bug fix. It needs `css/styles.css`, which this session was barred from. **It is the worst
   outstanding defect on the site and affects iPads in landscape.** Still unaddressed with the
   campaign starting w/c 2026-09-28.
2. **320px residuals** — unchanged, and unrelated to this session's pages.
3. **Session 5 input:** `entering-the-kimberley.html` no longer publishes a day rate or the travel
   disbursement line. Confirm `how-to-engage.html` covers both, or they are lost from the site.
4. **The security-of-payment treatment now appears in three places** — `kimberley-business.html`'s
